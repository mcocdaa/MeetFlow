...
class MeetingSeries(Base):
    ...
    default_host_user_id: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    default_recorder_user_id: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    status: Mapped[SeriesStatus] = mapped_column(Enum(SeriesStatus), default=SeriesStatus.active, index=True)
    ...
    default_host: Mapped[User | None] = relationship(foreign_keys=[default_host_user_id])
    default_recorder: Mapped[User | None] = relationship(foreign_keys=[default_recorder_user_id])
    ...
    participants: Mapped[list["SeriesParticipant"]] = relationship(
        back_populates="series", cascade="all, delete-orphan", order_by="SeriesParticipant.position"
    )
    standing_items: Mapped[list["StandingAgendaItem"]] = relationship(
        back_populates="series", cascade="all, delete-orphan", order_by="StandingAgendaItem.position"
    )
...
class SeriesParticipant(Base):
    ...
    series_id: Mapped[str] = mapped_column(ForeignKey("meeting_series.id", ondelete="CASCADE"), primary_key=True)
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), primary_key=True)
    participation_role: Mapped[ParticipationRole] = mapped_column(Enum(ParticipationRole), default=ParticipationRole.attendee)
...
class StandingAgendaItem(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    series_id: Mapped[str] = mapped_column(ForeignKey("meeting_series.id", ondelete="CASCADE"), index=True)
    title: Mapped[str] = mapped_column(String(240))
    agenda_type: Mapped[AgendaType] = mapped_column(Enum(AgendaType), default=AgendaType.discussion)
    default_owner_user_id: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    ...
    default_owner: Mapped[User | None] = relationship(foreign_keys=[default_owner_user_id])
...
class Meeting(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    project_id: Mapped[str] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), index=True)
    series_id: Mapped[str | None] = mapped_column(ForeignKey("meeting_series.id", ondelete="SET NULL"), nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(240), index=True)
    purpose_markdown: Mapped[str] = mapped_column(Text, default="")
    scheduled_start: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True)
    scheduled_end: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    status: Mapped[MeetingStatus] = mapped_column(Enum(MeetingStatus), default=MeetingStatus.draft, index=True)
    host_user_id: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    recorder_user_id: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    ...
    current_snapshot_id: Mapped[str | None] = mapped_column(
        ForeignKey("meeting_snapshots.id", use_alter=True, name="fk_meeting_current_snapshot"), nullable=True
    )
    version: Mapped[int] = mapped_column(Integer, default=1)
    created_by: Mapped[str] = mapped_column(ForeignKey("users.id"))
    updated_by: Mapped[str] = mapped_column(ForeignKey("users.id"))
    ...
    snapshots: Mapped[list["MeetingSnapshot"]] = relationship(
        back_populates="meeting", cascade="all, delete-orphan", foreign_keys="MeetingSnapshot.meeting_id"
    )
    ...
    amendments: Mapped[list["MeetingAmendment"]] = relationship(
        back_populates="meeting", cascade="all, delete-orphan", order_by="MeetingAmendment.created_at"
    )
...
class MeetingSnapshot(Base):
    ...
    meeting: Mapped[Meeting] = relationship(back_populates="snapshots", foreign_keys=[meeting_id])
    creator: Mapped[User] = relationship(foreign_keys=[created_by])
