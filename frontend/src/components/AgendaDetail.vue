...
const draft = reactive({ title: props.item.title, agenda_type: props.item.agenda_type as AgendaType, notes_markdown: props.item.notes_markdown, estimated_minutes: props.item.estimated_minutes as number | null })
const composer = ref<'decision' | 'action' | 'question' | null>(null)
const saving = ref(false)
const error = ref('')
const conflict = ref<{ version: number; server: string } | null>(null)

watch(() => props.item, (item) => Object.assign(draft, { title: item.title, agenda_type: item.agenda_type, notes_markdown: item.notes_markdown, estimated_minutes: item.estimated_minutes }), { deep: true })

async function save(expectedVersion = props.item.version) {
  saving.value = true
  error.value = ''
  try {
    await api(`/api/agenda-items/${props.item.id}`, { method: 'PUT', body: JSON.stringify({ expected_version: expectedVersion, title: draft.title.trim(), agenda_type: draft.agenda_type, notes_markdown: draft.notes_markdown, estimated_minutes: draft.estimated_minutes }) })
    conflict.value = null
    emit('changed')
  } catch (caught) {
...
<template>
  <section class="workspace-section agenda-detail" data-testid="agenda-detail">
    ...
    <div class="agenda-save-row"><span class="muted">版本 {{ item.version }}</span><button class="button button-quiet" :disabled="saving || !draft.title.trim()" @click="save()">保存议题</button></div>
