...
async function addAgenda() {
  ...
  try {
    const item = await api<AgendaItem>(`/api/meetings/${props.meeting.id}/agenda-items?expected_meeting_version=${props.meeting.version}`, {
      method: 'POST',
      body: JSON.stringify({ title: title.value.trim(), agenda_type: agendaType.value, notes_markdown: '', position: ordered.value.length }),
    })
    title.value = ''
    adding.value = false
    emit('select', item.id)
...
async function command(item: AgendaItem, action: 'skip' | 'cancel') {
  saving.value = true
  error.value = ''
  try {
...
<template>
  <aside class="agenda-queue" data-testid="agenda-queue">
    ...
    <div class="agenda-queue-list">
      <article v-for="(item, index) in ordered" :key="item.id" :data-testid="`agenda-row-${item.id}`" class="agenda-queue-row" :class="{ selected: item.id === selectedId }" draggable="true" @dragstart="startDrag(item.id)" @dragover.prevent @drop.prevent="dropOn(item.id)">
        <button class="agenda-select" @click="emit('select', item.id)"><span class="agenda-index">{{ index + 1 }}</span><span><strong>{{ item.title }}</strong><small>{{ item.status }} · {{ item.estimated_minutes ?? '—' }} 分钟</small></span></button>
        <div class="agenda-menu"><button class="agenda-menu-trigger" :aria-label="`议题“${item.title}”的更多操作`" :aria-expanded="menuId === item.id" @click="menuId = menuId === item.id ? '' : item.id">•••</button><div v-if="menuId === item.id"><button @click="emit('select', item.id); menuId = ''">编辑详情</button><button v-if="item.status === 'planned'" @click="command(item, 'skip')">跳过议题</button><button @click="command(item, 'cancel')">取消议题</button><button class="danger-link" @click="remove(item)">删除议题</button></div></div>
