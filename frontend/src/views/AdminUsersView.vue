...
const statusLabel: Record<UserStatus, string> = {
  pending: '待审批', active: '已启用', rejected: '已拒绝', disabled: '已禁用',
}
...
<template>
  <main class="page">
    ...
    <section class="panel">
      <div class="section-heading"><h2>成员列表</h2><button class="button button-quiet" @click="load">刷新</button></div>
      ...
      <div v-else class="table-list">
        <article v-for="user in users" :key="user.id" class="user-row">
          ...
          <div class="row-actions">
            ...
            <button v-if="user.status === 'active' && user.role !== 'admin'" class="button button-small button-danger" @click="transition(user.id, 'disable')">禁用</button>
