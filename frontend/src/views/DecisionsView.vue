...
<template>
  <main class="workspace-page"><header class="workspace-page-heading"><div><p class="eyebrow">Decision log</p><h1>决策日志</h1><p>跨项目查看提案、最终决策及其评审状态。</p></div><span class="metric"><strong>{{ total }}</strong>项</span></header>
    ...
    <section v-else-if="visible.length" class="global-record-list"><article v-for="item in visible" :key="item.id" class="workspace-section global-record"><div><StatusPill :status="item.status" kind="decision" /><span class="muted">{{ projectNames[item.project_id] ?? item.project_id }}</span></div><h2>{{ item.title }}</h2><p>{{ item.decision_markdown }}</p><footer><span>{{ formatDateTime(item.updated_at) }}</span><RouterLink v-if="item.meeting_id" :to="`/meetings/${item.meeting_id}`">查看来源会议 →</RouterLink></footer></article></section>
