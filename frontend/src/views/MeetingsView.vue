...
<template>
  <main class="workspace-page"><header class="workspace-page-heading"><div><p class="eyebrow">Meeting workspace</p><h1>会议</h1><p>围绕项目组织准备、现场议题、产出和会后档案。</p></div><button class="button button-primary" @click="createOpen = !createOpen">{{ createOpen ? '收起' : '新建会议' }}</button></header>
    <form v-if="createOpen" class="workspace-section meeting-create-form" @submit.prevent="createMeeting"><header class="section-heading"><h2>创建项目会议</h2><button type="button" class="icon-button" aria-label="关闭" @click="createOpen = false">×</button></header><div class="form-grid"><label>所属项目<select v-model="form.project_id" required><option value="" disabled>选择项目</option><option v-for="project in projects" :key="project.id" :value="project.id">{{ project.name }}</option></select></label><label>会议标题<input v-model.trim="form.title" required /></label><label>开始时间<input v-model="form.scheduled_start" type="datetime-local" required /></label><label>结束时间<input v-model="form.scheduled_end" type="datetime-local" required /></label><label class="span-2">会议目的<textarea v-model="form.purpose_markdown" rows="3" /></label></div><div class="form-actions"><button class="button button-primary" :disabled="creating">{{ creating ? '创建中…' : '创建会议' }}</button></div></form>
