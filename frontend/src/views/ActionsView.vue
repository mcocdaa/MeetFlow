...
<template>
  <main class="workspace-page"><header class="workspace-page-heading"><div><p class="eyebrow">Action hub</p><h1>行动项</h1><p>会议产生的行动在这里汇总、筛选并持续跟踪。</p></div><span class="metric"><strong>{{ total }}</strong>项</span></header>
    ...
    <section v-else-if="visible.length" class="global-record-list"><article v-for="item in visible" :key="item.id" class="workspace-section action-record"><span class="action-check">{{ item.status === 'done' ? '✓' : '○' }}</span><div><div class="tag-row"><span class="tag" :data-priority="item.priority">{{ priorityLabel(item.priority) }}</span><StatusPill :status="item.status" kind="action" /><span class="tag tag-project">{{ projectNames[item.project_id] ?? item.project_id }}</span></div><h2>{{ item.content }}</h2><p>{{ item.owner_user_id === session.user?.id ? '负责人：我' : item.owner_user_id ? `负责人：${item.owner_user_id}` : '未指定负责人' }} · {{ item.due_date ? `截止 ${item.due_date}` : '未设截止日期' }}</p></div><RouterLink v-if="item.meeting_id" :to="`/meetings/${item.meeting_id}`">来源会议 →</RouterLink></article></section>
