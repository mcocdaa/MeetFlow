...
<template>
  <main class="workspace-page">
    ...
    <form v-if="createOpen" class="panel project-create" @submit.prevent="createProject">
      <label>项目名称<input v-model.trim="form.name" required /></label><label>项目标识<input v-model.trim="form.slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="meetflow" required /></label><label class="grow">一句话说明<input v-model.trim="form.summary" /></label><button class="button button-primary" :disabled="creating">{{ creating ? '创建中…' : '创建' }}</button>
    </form>
