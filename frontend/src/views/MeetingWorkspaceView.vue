    ...
    <template v-else-if="meeting">
      ...
      <template v-else>
        ...
        <section class="workspace-section meeting-summary-section">
          <header class="section-heading"><div><p class="eyebrow">Summary</p><h2>会议纪要</h2></div><div class="row-actions"><span v-if="minutesSaved || saveState === 'saved'" class="muted" role="status">纪要已保存</span><button class="button button-quiet" :disabled="busy || !dirty" @click="saveMinutes">保存会议纪要</button></div></header>
          <PluginEditorSlot
            editor-label="会议纪要"
